...
export function usePreviewChat(contentId: bigint, previewText: string) {
    ...
    async (text: string) => {
      ...
      try {
        const res = await fetch("/api/chat", {
          ...
          body: JSON.stringify({ contentId: contentId.toString(), previewText, messages: nextMessages }),
        });
  ...
  const remaining =
    MAX_MESSAGES - parseInt(typeof window !== "undefined" ? sessionStorage.getItem(SESSION_KEY) || "0" : "0", 10);
